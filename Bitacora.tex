...
% Actividad de Merayo
\noindent \textbf{Con respecto al audio:} \\


% Actividad de Keylor
\noindent \textbf{Con respecto al picoblaze:} \\
...
\begin{flushright}
	\begin{large}
		\textbf{Fecha: 25 de Octubre}\\[5ex]
	\end{large}
\end{flushright}

\noindent \textbf{Integrantes:} Luis Merayo, Luis Leon, Keylor Mena \\[1ex]
\textbf{Hora:} 14:00 - 14:30\\[1ex]
\textbf{Actividad:} \\[2ex]

Se hicieron las primeras pruebas de los módulos en la FPGA, dando un resultado negativo en cuanto al funcionamiento de todo el sistema. Sin embargo, se detectó un error en la VGA que puede estar afectando al circuito implementado. \\
Además, se corrigió un problema de direcciones en la VGA, que no concordaban con las establecidas en el microcontrolador. Asimismo, se cambió el sistema de acceso a la misma debido a incompatibilidades, ya que no se concordó en buena comunicación entre los miembros durante el desarrollo del microcontrolador y la VGA.

% Nueva entrada
\begin{flushright}
	\begin{large}
		\textbf{Fecha: 27 de Octubre}\\[5ex]
	\end{large}
\end{flushright}

\noindent \textbf{Integrantes:} Luis Merayo, Luis Leon \\[1ex]
\textbf{Hora:} 13:00 - 19:30\\[1ex]
\textbf{Actividad:} \\[2ex]

Se detectó un problema en el teclado que no permitía que escribiera en el microcontrolador. El error fue detectado en el archivo ucf, donde no se colocaban los pullup a los pines de comunicación con el integrado adaptador PS2.
