...
\fancyhead[L]{\begin{tabular}{l r | l r}	
	...
	\begin{document}
		...
		\begin{figure}[htbp]
			...
			\caption[3erNivel]{Proceso general de operación del circuito}
			\label{fig:VG}
		\end{figure}
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=16cm]{img/irqactivado.jpeg}
			\caption[3erNivel]{Señal IRQ activada}
			\label{fig:IRQ}
		\end{figure}
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=16cm]{img/timeroff_irqoff.jpeg}
			\caption[3erNivel]{Señal IRQ desactivada y alarma desactivada}
			\label{fig:OFF}
		\end{figure}
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=16cm]{img/puestaydireccion.jpeg}
			\caption[3erNivel]{Escritura de dirección en la RTC}
			\label{fig:DIR}
		\end{figure}
		...
		\begin{itemize}
			\item El tiempo no esta en una optima condición, en la tabla \ref{tab:consumo} se puede notar que el tiempo de ejecución esta en 98.739 MHz.
			\item En las mediciones realizadas en el analizador lógico podemos notar que las señales RD,CS y WR no tienen mucha separacion en sus comienzos y finales.
		\end{itemize}
		...
		Para las conexiones del diseño implementado en Verilog del circuito, se debe seguir con la siguiente guía de conexión, que en este caso, se muestran en las tablas \ref{tab:Espec} y \ref{tab:Espec1}. Es importante seguirla para evitar conflictos y cabe aclarar que se recomienda implementarla en Nexys 4, ya que esta tabla tiene el pinout establecida para dicha placa de FPGA.
		...
		\begin{table}[htbp]
			...
			\begin{tabular}{c||c||c||c}
				...
				PS2\_CLOCK & input & F4 & Señal de reloj del teclado\\
		...
		\begin{table}[htbp]
			\centering
			\caption{Continuación de asignación de pines de las entradas y salidas}
			\begin{tabular}{c||c||c||c}
				Señal & Designación & Pin & Función\\
				\hline
				\hline
	...
	\end{table}
		...
		\begin{itemize}
			\item Utiliza un microcontrolador como máquina de control facilitó el proceso de desarrollo de dicha etapa, debido a que se abstrae a nivel de software.
			\item Colocar un módulo externo de RTC aliviana la carga del microcontrolador en algunos sistemas, como por ejemplo, en dispositivos móviles, ya que se puede aprovechar el potencial del microprocesador para ejecutar tareas de mayor importancia.
			\item Desarrollar una unidad de control principal ayuda en la comunicación entre los diferentes periféricos, esto comunicando un identificador (PORT\_ID) y un bus de datos centralizado.
			\item Colocar un teclado para configurar la información del sistema ayuda a tener una mejor experiencia de usuario, ya que se pueden escribir los números en lugar de estar seleccionándolos mediante dos botones. 

		\end{itemize}
		
		\subsection{Recomendaciones}
		\begin{itemize}
			\item Se debe acondicionar mejor la comunicación entre la RTC y la Nexys 4, debido a fluctuaciones en la alimentación de este en el IRQ que ocasionaron problemas. Esto se solucionó de forma temporal mediante un circuito RC.
			\item La interfaz de la VGA debe realizarse de forma externa para mejorar la experiencia, ya que, si se importan imágenes mediante ROM en HDL, puede consumir gran cantidad de recursos que pueden afectar la velocidad máxima del sistema.
			\item Controlar los periféricos mediante circuitos modulares separados.
		\end{itemize}
